        ...
        <div class="container py-5">
           <div class="d-flex justify-content-around mb-5">
                ...
                <form method="GET" class="d-flex ms-5 search-form" name="search"  action="{{ route('products.index') }}">
                    <input class="form-control me-2 rounded-pill" type="search" name="search" placeholder="Nhập tên sản phẩm để tìm" aria-label="Search">
                ...
                <table class="table hid-border-style">
                    ...
                    <tbody class="border-bottom">
                        ...
                        <tr class="text-center border-top">
                            <td class="py-5">
                                <img src="{{$product->img ? asset('upload/products/' .$product->img) : asset('upload/products/default.png') }}" style="width: 140px; height: 90px;" alt="Hình ảnh">
                            </td>
                            <td class="py-5">{{$product->name}}</td>
                            <td class="py-5">{{$product->productdetails->first()->price ?? '0'}}</td>
                            <td class="py-5">{{$product->productdetails->first()->quanity ?? '0'}}/{{$product->productdetails->first()->unit ?? '_'}}</td>
                            <td class="py-5">{{ $product->brand ? $product->brand->name : '' }}</td>                                
                            <td class="py-5 d-flex justify-content-around">
                                <a href="{{route('products.show', $product->id)}}" class="btn"><i class="fa-solid fa-eye text-secondary"
                                            style="font-size: 1.25rem;"></i></a>
                                    <a href="{{route('products.edit', $product->id)}}" class="btn"><i class="fa-solid fa-pen text-primary"
                                            style="font-size: 1.25rem;"></i></a>
